...
public class CryptUtils {
    ...
    private static final String ADD_STRING = "DECRYPT";
    ...
    public static String decryptWritibleString (String str){
        String decodeStr = new String(decryptString(Base64.decode(str,Base64.DEFAULT)));
        decodeStr = decodeStr.replace(ADD_STRING,"");
        return decodeStr;
    }
    ...
    public static byte[]  decryptString (byte [] crypt){
        PreferencesManager preferencesManager = PreferencesManager.getInstance();
        String key = preferencesManager.getCryptKey();
        if (key.length()!=32){
            for (int i=key.length();i<32;i++){
        ...
        return Process(crypt,bytesKey,sBox,false);
    }
